...
See also `docs/known-issues.md` (export-governance review findings, most now addressed — kept as the historical record) and `docs/plans/safety-plan.md` (the design doc for that work, §-numbered, useful for "why does this field/table exist" questions). For the DMZ-proxy PII boundary specifically (MRNs, dates, DICOM UIDs, filesystem paths never crossing to the browser, including in error states), see `docs/pii-boundary-safety.md` (the risk register) and `docs/plans/pii-boundary-test-suite.md` (the CI-gated test suite + remediation plan that closes it).
