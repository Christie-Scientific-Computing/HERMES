"""
Exercises deps.get_session / auth.login_user / auth.logout_user through real
HTTP request/response cycles against the throwaway app conftest.py builds --
see that module's docstring for why Phase 0 needs its own test-only routes.
"""
...
def test_login_without_remember_omits_cookie_max_age(client, make_user):
    ...
    resp = client.post("/test/login", data={"username": "alice", "remember": "false"})
    assert resp.status_code == 200
    set_cookie = resp.headers.get("set-cookie")
    assert "hermes_session=" in set_cookie
    assert "max-age" not in set_cookie.lower()
...
def test_login_with_remember_sets_cookie_max_age(client, make_user):
    make_user(username="alice")
    resp = client.post("/test/login", data={"username": "alice", "remember": "true"})
    set_cookie = resp.headers.get("set-cookie")
    assert "hermes_session=" in set_cookie
...
def test_require_data_custodian_allows_staff_user(client, make_user):
    ...
    client.post("/test/login", data={"username": "bob", "remember": "true"})
    resp = client.get("/test/staff-only")
    assert resp.status_code == 200
