...
async def local_pacs_series(study_instance_uid: str, request: Request, user: User = Depends(require_login)):
    ...
    try:
        ...
        error = str(e)
    return templates.TemplateResponse(request, "local_pacs/_series.html", {"series": series, "error": error})
...
async def local_pacs_move(
    ...
    try:
        result = await asyncio.to_thread(
            cc.move_study, study_instance_uid=study_instance_uid, destination_ae=destination.ae_title,
        )
    ...
    try:
        await backend_client.audit_local_pacs_move(
            ...
            performed_by=user.username, outcome=outcome, detail=detail,
        )
        audit_warning = ""
    except backend_client.BackendError:
        logger.exception("Local PACS move audit call failed for study %s", study_instance_uid)
        audit_warning = " (warning: the audit record for this move could not be saved)"
