...
mode = st.radio('Mode', ['From CSV upload', 'By job id'], index=0)
...
if 'patients' in st.session_state:
    ...
    patient_filter = st.checkbox("Show failed only", value=False)
    ...
    for mrn in patients:
        try: 
            if mode == 'By job id' and 'job_id' in locals() and job_id:
                ...
                res = requests.get(f"{BASE_URL}/results/patient/timeline/{mrn}/all")
